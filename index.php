	    ...
	    <script type="text/javascript">	
	    	$(function (){
				...
				$("#regForm").on("submit", function(e){					
					...
					// if(document.getElementById("strengthBar").getAttribute("data-strength")<8.0){  UNCOMMENT
					// 	displayMessage("Password is not strong enough.");							UNCOMMENT
					// 	document.getElementById("password").style.border=INVALID_MISSING_BORDER;	UNCOMMENT
					// }else{	UNCOMMENT
						var form = $(this);
						...
						request.done(function(response, textStatus, jqXHR){
							$.getScript("js/indexPageScripts.js", function(){
								...
								if(response.missingData.length==0){
									...
									if(response.invalidData.length==0){
											...
											if(response.existingData.length==0){
												$("#regMessageBox").css("visibility", "visible");
												$("#regMessageBox").removeClass("alert-danger");
												$("#regMessageBox").addClass("alert-success");
												$("#regMessage").html("Sign up success!");
									...
									missingData(response.missingData, "regMessageBox", "regMessage");
								}
							});
						...
						if(response.missingData.length==0){
							if(response.userExists){
								if(response.correctPassword){
									displayMessage("Successfully logged in" , "loginMessageBox", "loginMessage");
									window.location.href = "homepage.php";
								}else{
									displayMessage("Incorrect password","loginMessageBox", "loginMessage" );
								}
							}else{
								displayMessage("No such username exists", "loginMessageBox", "loginMessage");
							}
						}else{
							missingData(response.missingData, "loginMessageBox", "loginMessage");
						}
						console.log("missing data = " + response.missingData);
						console.log("user exists = " + response.userExists);
						console.log("correct password = " + response.correctPassword);

					});

					request.fail(function(jqXHR, textStatus, errorThrown){
						console.log("Whoopsie! Login failed");
					});
					...
					<div id="cardForm" class="card bg-dark text-light">
						...
						<div class="card-body">
							<form id="loginForm" method="post" target="_blank">
								<input id="login_username" type="text" class="form-control loginInput" name="login_username" placeholder="Username" autofocus required oninvalid="setCustomValidity('Please enter a username')"/><br>
							 	<input id="login_password" type="password" class="form-control loginInput" name="login_password" placeholder="Password" required oninvalid="setCustomValidity('Please enter a password')"/><br>

							 	<p class="card-text" style="float:left;">Click <a class="card-link" href="">here</a> if you've forgotten your password</p>
							 	<input type="submit" class="btn btn-light"/>
							</form>
							<div id="loginMessageBox" class="alert alert-danger alert-dismissible">
								<p id="loginMessage"></p>
								...
								<form id="regForm" method="post" target="_blank">
									<div class="form-group">
										...
										<div id="usernamePopover" class="regPopovers" data-placement="top" data-content="Username can only contain lowercase and uppercase letters, numbers, underscores and hyphens." data-trigger="hover">
											<input type="text" id="username" class="form-control regInput" name="username" autofocus required oninvalid="setCustomValidity('Please enter a username')"/><br>
										</div>
									</div>
									<div class="form-group">
										<label for="email" class="label">Email address:</label>
							 			<input type="email" id="email" class="form-control regInput" name="email" required oninvalid="setCustomValidity('Please enter a valid email address')"/><br>
									</div>
									<div class="form-group">
										...
										<div id="passwordPopover" class="regPopovers" data-placement="top" data-content="Password must have strength of at least 8 by using mixture of uppercase and lower case letters, numbers and punctuation." data-trigger="hover">
							 				<input type="password" id="password" class="form-control regInput" name="password" required oninvalid="setCustomValidity('Please enter a valid password')" onkeyup="checkPass(this.value)" /><br>
							 			</div>
									...
									<div class="form-group">
										<label for="confirm_password" class="label">Confirm password:</label>
							 			<input type="password" id="confirm_password" class="form-control regInput" name="confirm_password"/><br>
									</div>
							 		<input type="submit" id="submit" name="submit" value="Sign up" class="btn btn-dark"/>
								</form>
								<div id="regMessageBox" class="alert alert-danger alert-dismissible">
									<p id="regMessage"></p>
									<button type="button" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								</div>
							</div>
